...
default_config = {
    ...
    "loggers": {"root": {"level": "INFO", "handlers": ["consoleHandler"]}},
    "handlers": {
        "consoleHandler": {
    ...
    "formatters": {
        "json": {
            ...
            "timestamp": True,
            "static_fields": {"app": "dyn-dns-client"},
...
if os.path.isfile(default_logging_config):
    with open(default_logging_config, "r") as stream:
        try:
            ...
            print(
                f"WARNING: Failed to load logging configuration file. Will use default logging parameters.",
                file=sys.stderr,
            )
            print(
                f"########################################################################################",
