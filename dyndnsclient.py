...
class DynDnsClient:
    ...
    class PythonDnsResolverWrapper(DnsResolver):
        ...
        def resolve(self, dns_name: str) -> IPv4Address:
            ...
            answer = self._dns_resolver.resolve(qname=dns_name, rdtype=dns.rdatatype.A, raise_on_no_answer=True)
            rrset = [record.to_text() for record in answer.rrset]
            
            if (len(rrset) > 1):
                raise ValueError(f"DNS A record for {dns_name} has more than one IP address. This is not supported.")
            if (len(rrset) < 1):
                return None
            ...
            try:
                ...
                if (not isinstance(ip, IPv4Address)):
                    logger.error(f"Obtained IP address ({rrset[0]}) is not a valid IPv4 address")
                else:
                    return ip
            except:
    ...
    def update_dns_record(self, ipv4: IPv4Address) -> None:
        ...
        if ((now - self._last_dns_query_timestamp) >= self._dns_cache_ttl_sec):
            logger.info(f"Local DNS cache for {self._dns_name} has expired.")
            self._current_ip = self._query_dns(dns_name=self._dns_name)
        ...
        logger.info(f"The Public IP has changed from {self._current_ip} to {ipv4}", extra={'ip_change': 1})

        success = self._update_dns_record(dns_name=self._dns_name, ipv4=ipv4.exploded)

        if success:
            self._current_ip = ipv4


    def _query_dns(self, dns_name: str) -> str:
        logger.info(f"Performing a DNS query for {dns_name} to obtain the current IP address.", extra={'dns_query': 1})
        return self._dns_resolver.resolve(dns_name=dns_name)

    def _update_dns_record(self, dns_name: str, ipv4: str):
        if (self._id_token is None or self._id_token.expired):
            logger.info("An Id Token needs to be generated to call the DynDNS Function.")
        ...
        try:
            response = requests.post(url=self._dyn_dns_api_url, headers=headers, data=json.dumps(data))
            response.raise_for_status()
            logger.info(f"Successfully updated the {dns_name} DNS A record with IP {ipv4}", extra={"status_code": response.status_code})
    ...
    def _get_id_token(self) -> google.oauth2.service_account.IDTokenCredentials:
        ...
        id_token = google.oauth2.id_token.fetch_id_token_credentials(audience=self._dyn_dns_api_url)
